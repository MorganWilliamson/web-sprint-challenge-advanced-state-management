...
Context API is a quicker, cleaner way to manage state, although it should probably be contained to smaller applications rather than larger-scale projects. 

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
The store contains our immutable state for the rest of the app to draw from, the idea being that the information held within will stay immutable no matter what changes outside of the store. Actions and reducers are used to influence changes to the state, with switch cases and action types/creators/objects, without directly changing the original state. 

3. What is the difference between Application state and Component state? When would be a good time to use one over the other?
Application state is the global state of the application, where as component state is only local to the component it's being used in. The question to ask yourself is whether this particular slice of state is going to be used globally, or just within a singular component. Your answer will determine what availability a particular slice of state will need.

4. Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
thunk is a redux middleware that helps us asynchronously dispatch our action(s), and changes our action creators so that they can return a function rather than just an object. 

5. What is your favorite state management system you've learned and this sprint? Please explain why!
Redux thunk is probably my favorite, but Context API is really simple and that's nice. I'm not sure that I have a favorite yet, I'd like to spend more time working with them before I can make that decision. 
...
- [*] Retrieve an array of all of the Smurfs in the Smurf DB by writing a `GET` to the endpoint `/smurfs`.
